...
class AdminBlockUserView(APIView):
    ...
    def post(self, request, user_id):
        ...
        # Check if this is an unblock request based on the URL path
        is_unblock = 'unblock' in request.path
        
        # Update user's active status
        user.is_active = True if is_unblock else False
        user.save()
...
class AdminPaymentListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Serialize data with request context for absolute avatar URLs
            serializer = PaymentSerializer(paginated_queryset, many=True, context={'request': request})
...
class AdminSubscriptionListView(APIView):
    ...
    def get(self, request):
        try:
            from marketplace.payment_serializers import UserSubscriptionSerializer
            ...
            # Serialize data with request context for avatar URLs
            serializer = UserSubscriptionSerializer(paginated_queryset, many=True, context={'request': request})
            ...
            logger.error(f"Error in AdminSubscriptionListView: {str(e)}", exc_info=True)
            return Response({
                "success": False,
    ...
    def patch(self, request):
        """Uncancel a user's subscription (admin only)"""
        try:
            ...
            return Response({
                "success": True,
                "message": f"Subscription restored successfully for {subscription.user.username}",
                "data": {
            ...
            logger.error(f"Error uncanceling subscription: {str(e)}", exc_info=True)
            return Response({
                "success": False,
                "message": "Error uncanceling subscription",
                "error": str(e)
            }, status=500)
    
    def delete(self, request):
        """Cancel or permanently delete a user's subscription (admin only)"""
        try:
            subscription_id = request.data.get('subscription_id')
            ...
            plan_name = subscription.plan.display_name if subscription.plan else "Free Plan"
            ...
            if permanent_delete:
                ...
                if subscription.stripe_subscription_id:
                    try:
                        ...
                        logger.warning(f"Failed to delete Stripe subscription: {str(stripe_error)}")
                
                subscription.delete()
                logger.info(f"Admin permanently deleted subscription {subscription_id} for user {user_info['username']}")
                
                return Response({
                    "success": True,
                    "message": f"Subscription permanently deleted for {user_info['username']}",
                    "data": {
                        "subscription_id": subscription_id,
                        "user": user_info,
            ...
            logger.error(f"Error in subscription delete operation: {str(e)}", exc_info=True)
            return Response({
                "success": False,
                "message": "Error processing subscription delete request",
                "error": str(e)
            }, status=500)
