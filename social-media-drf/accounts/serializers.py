...
class UserSerializer(serializers.ModelSerializer):
    ...
    is_blocked_locally = serializers.SerializerMethodField(read_only=True)
    
    class Meta:
        model = User
        fields = ['id', 'username', 'email', 'avatar', 'display_name', 'is_online', 'last_seen', 'is_active', 'is_blocked_locally']
    ...
    def get_is_blocked_locally(self, obj):
        """Check if this user is blocked by the current request user"""
        request = self.context.get('request')
        if not request or not request.user.is_authenticated:
            return False
        
        # In the context of the report modal, we usually want to know if the 
        # REPORTED user is blocked by the REPORTER.
        # However, for a general serializer, we might want to know if blocked by the *viewer*.
        # For admin unified reports, we need to pass the reporter context.
        reporter_id = self.context.get('reporter_id')
        if reporter_id:
            from chats.models import BlockedUser
            return BlockedUser.objects.filter(blocker_id=reporter_id, blocked=obj).exists()
