...
class RoomViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if blocked_user_ids:
            rooms = rooms.exclude(is_group=False, participants__id__in=blocked_user_ids)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if participant_id:
            ...
            if BlockedUser.objects.filter(
                ...
                return Response(
                    {"success": False, "error": "You have blocked this user"},
            ...
            if BlockedUser.objects.filter(
                ...
                return Response(
                    {"success": False, "error": "This user has blocked you"},
                    status=status.HTTP_403_FORBIDDEN,
                )
        ...
        if not name or not name.strip():
            return Response(
                {"success": False, "error": "Room name is required for group chats"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if member_ids:
            try:
                members = User.objects.filter(id__in=member_ids).exclude(
                    id=request.user.id
                )
    ...
    def messages(self, request, pk=None):
        ...
        if request.user not in room.participants.all():
            return Response(
                {"success": False, "error": "You don't have access to this room"},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def delete_message(self, request, pk=None):
        ...
        message_id = request.query_params.get("message_id") or request.data.get(
            "message_id"
        )
    ...
    def add_members(self, request, pk=None):
        ...
        if not room.is_group:
            return Response(
                {"success": False, "error": "Can only add members to group rooms"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not member_ids:
            return Response(
                {"success": False, "error": "member_ids is required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def make_admin(self, request, pk=None):
        ...
        if not room.is_group:
            return Response(
                {"success": False, "error": "Can only manage admins for group rooms"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not room.is_admin(request.user):
            return Response(
                {"success": False, "error": "Only admins can make other users admin"},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def remove_member(self, request, pk=None):
        ...
        if not room.is_group:
            return Response(
                {"success": False, "error": "Can only remove members from group rooms"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def toggle(self, request):
        ...
        if message.room:
            if request.user not in message.room.participants.all():
                return Response(
                    {"success": False, "error": "You don't have access to this chat"},
                    status=status.HTTP_403_FORBIDDEN,
                )
        else:
        ...
        if channel_layer:
            ...
            if message.room:
                async_to_sync(channel_layer.group_send)(
                    f"chat_{message.room.id}", broadcast_data
                )
            else:
                async_to_sync(channel_layer.group_send)(
                    f"user_{message.sender.id}", broadcast_data
                )
                async_to_sync(channel_layer.group_send)(
                    f"user_{message.receiver.id}", broadcast_data
                )
...
class ChatUserListView(APIView):
    ...
    def get(self, request):
        ...
        # Get IDs of users blocked by current user
        blocked_user_ids = BlockedUser.objects.filter(blocker=request.user).values_list(
...
class SendDirectMessageView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(
                f"Message check - User {request.user.id} to {receiver.id}: can_message={can_message}, has_previous_messages={has_previous_messages}"
            )
        except Exception as e:
        ...
        if not can_message and not has_previous_messages:
            ...
            rejected_request = MessageRequest.objects.filter(
                sender=request.user, receiver=receiver, status="rejected"
            ).exists()
...
class GetMessageRequestsView(APIView):
    ...
    def get(self, request):
        pending_requests = (
            MessageRequest.objects.filter(receiver=request.user, status="pending")
        ...
        return Response(
            {
                ...
                "data": {
                    "request": MessageRequestSerializer(
                        message_request, context={"request": request}
                    ).data,
...
class CancelMessageRequestView(APIView):
    ...
    def delete(self, request):
        request_id = request.query_params.get("request_id") or request.data.get(
            "request_id"
        )
        ...
        try:
            message_request = MessageRequest.objects.get(
                id=request_id, sender=request.user, status="pending"
            )
        except MessageRequest.DoesNotExist:
            return Response(
                {
        ...
        return Response(
            {"success": True, "message": "Message request cancelled successfully."},
            status=status.HTTP_200_OK,
        )
    ...
    def get(self, request):
        ...
        pending_request_received = (
            MessageRequest.objects.filter(
                sender=other_user, receiver=request.user, status="pending"
            )
            .select_related("sender", "receiver")
            .order_by("-created_at")
            .first()
        ...
        pending_request_sent = (
            MessageRequest.objects.filter(
                sender=request.user, receiver=other_user, status="pending"
            )
            .select_related("sender", "receiver")
            .order_by("-created_at")
            .first()
        ...
        if pending_request_received:
            response_data["pending_request"] = MessageRequestSerializer(
                pending_request_received, context={"request": request}
            ).data
            response_data["has_pending_request"] = True
            response_data["is_request_receiver"] = True
        elif pending_request_sent:
            response_data["pending_request"] = MessageRequestSerializer(
                pending_request_sent, context={"request": request}
            ).data
            response_data["has_pending_request"] = True
...
class GetConversationsListView(APIView):
    ...
    def get(self, request):
        ...
        sent_request_receivers = (
            MessageRequest.objects.filter(sender=request.user, status="pending")
            .values_list("receiver_id", flat=True)
            .distinct()
        )
        ...
        user_ids = set(
            list(sent_to)
            + list(received_from)
            + list(pending_request_senders)
            + list(sent_request_receivers)
        )
        ...
        for user_id in user_ids:
            try:
                ...
                conversation_data = {
                    "user": UserSerializer(
                        other_user, context={"request": request}
                    ).data,
                    "last_message": MessageSerializer(
                        last_message, context={"request": request}
                    ).data
                    if last_message
                    else None,
                ...
                if pending_request_received:
                    conversation_data["pending_request"] = MessageRequestSerializer(
                        pending_request_received, context={"request": request}
                    ).data
...
class UnblockUserView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"success": True, "message": f"You have unblocked {username}"},
            status=status.HTTP_200_OK,
        )
...
class ReportUserView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            report_serializer = UserReportSerializer(
...
class UpdateReportStatusView(APIView):
    ...
    def patch(self, request, report_id):
        ...
        # Only allow 'resolved' or 'dismissed' status updates
        if new_status and new_status in ["resolved", "dismissed"]:
        ...
        return Response(
            {
                "success": False,
                "error": "Invalid status. Only 'resolved' and 'dismissed' are allowed.",
            },
            status=status.HTTP_400_BAD_REQUEST,
        )


class AdminLocalBlockView(APIView):
    """Admin action to block a user on behalf of another user (e.g., blocker=reporter, blocked=reported)"""
    ...
    def post(self, request):
        ...
        if not blocker_id or not blocked_id:
            return Response(
                {
                    "success": False,
                    "error": "Both blocker_id and blocked_id are required",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            user_to_block = User.objects.get(id=blocked_id)
        except User.DoesNotExist:
            return Response(
                {"success": False, "error": "User not found"},
                status=status.HTTP_404_NOT_FOUND,
        ...
        if user_to_block == blocker:
            return Response(
                {"success": False, "error": "A user cannot block themselves"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Check if already blocked
        blocked_obj, created = BlockedUser.objects.get_or_create(
            blocker=blocker, blocked=user_to_block
        )

        if not created:
            return Response(
                {"success": False, "error": "User is already blocked by this person"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response(
            {
                "success": True,
                "message": f"Successfully blocked {user_to_block.username} for {blocker.username}",
            },
            status=status.HTTP_201_CREATED,
        )


class AdminLocalUnblockView(APIView):
    """Admin action to unblock a user on behalf of another user"""
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {
                    "success": True,
                    "message": "User unblocked successfully for the reporter",
                },
                status=status.HTTP_200_OK,
            )
        except Exception as e:
...
class AdminAllConversationsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            user_pairs = (
                Message.objects.filter(room__isnull=True)
            ...
            for sender_id, receiver_id in user_pairs:
                ...
                try:
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"Error processing direct conversation pair {pair_key}: {str(e)}"
                    )
                    continue
            ...
            if type_filter and type_filter != "all":
                all_conversations = [
                    c for c in all_conversations if c.get("type") == type_filter
                ]
                ...
                if search_query:
                    ...
                    for conv in all_conversations:
                        if conv.get("type") == "direct":
                            ...
                            user1_name = (
                                user1.get("display_name") or user1.get("username") or ""
                            ).lower()
                            user2_name = (
                                user2.get("display_name") or user2.get("username") or ""
                            ).lower()
                            user1_email = (user1.get("email") or "").lower()
                            user2_email = (user2.get("email") or "").lower()
                            if (
                                search_query in user1_name
                                or search_query in user2_name
                                or search_query in user1_email
                                or search_query in user2_email
                            ):
                            ...
                            if search_query in room_name or any(
                                search_query in name for name in participant_names
                            ):
                                filtered_conversations.append(conv)
            ...
            return Response(
                {
                    ...
                    "results": {"success": True, "data": paginated_conversations},
                }
            )
        except Exception as e:
            import logging
...
class AdminGetConversationMessagesView(APIView):
    ...
    def get(self, request):
        ...
        if conversation_type == "direct":
            ...
            return Response(
                {"success": False, "error": "type must be 'direct' or 'room'"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AdminCleanupConversationView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"success": False, "error": f"Date parsing error: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if conversation_type == "direct":
            ...
            deleted_count, _ = Message.objects.filter(
                room=room, created_at__range=(start_date, end_date)
            ).delete()
