...
def handle_checkout_completed(session):
    ...
    if payment_type == 'subscription':
        ...
        # Get or create user subscription record for history
        # If this stripe_subscription_id already exists for this user, we use it. 
        ...
        if not user_subscription:
            # Mark existing active subscriptions as 'completed'
            UserSubscription.objects.filter(user=user, status='active').update(status='completed')
            
            # Fetch customer ID if possible
            existing_sub = UserSubscription.objects.filter(user=user).order_by('-created_at').first()
            customer_id = existing_sub.stripe_customer_id if existing_sub else session.get('customer')
...
def handle_subscription_created(subscription):
    ...
    user = None
    user_subscription = UserSubscription.objects.filter(stripe_subscription_id=subscription_id).first()
    
    if user_subscription:
        user = user_subscription.user
    else:
        # Try to find any previous subscription with this customer_id to find the user
        prev_sub = UserSubscription.objects.filter(stripe_customer_id=customer_id).first()
        if prev_sub:
            user = prev_sub.user
        else:
    ...
    # For history: Create a new record if this subscription_id is not already tracked
    if not user_subscription:
        # Mark other active as completed
        UserSubscription.objects.filter(user=user, status='active').update(status='completed')
