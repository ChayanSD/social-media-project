...
const CommunityMembersModal: React.FC<CommunityMembersModalProps> = ({
  ...
  const getRoleBadge = (role?: string) => {
    switch (role) {
      case 'admin':
        return (
          <span className="px-2 py-1 rounded-full text-sm bg-purple-500/20 text-purple-400 border border-purple-500/30">
            Admin
          </span>
        );
      case 'moderator':
        return (
          <span className="px-2 py-1 rounded-full text-sm bg-blue-500/20 text-blue-400 border border-blue-500/30">
            Moderator
          </span>
        );
      default:
        return (
          <span className="px-2 py-1 rounded-full text-sm bg-white/10 text-white/60 border border-white/20">
            Member
          </span>
        );
    }
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-white/60 text-base mb-1">@{username}</p>
